...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] MapNetworkObject = null;
		public GameObject[] MinigameManagerNetworkObject = null;
		public GameObject[] NetworkCameraNetworkObject = null;
		public GameObject[] PlayerNetworkObject = null;
		public GameObject[] RopeNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is MapNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MapNetworkObject.Length > 0 && MapNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MapNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MapBehavior>();
						}
					}
			...
			else if (obj is MinigameManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MinigameManagerNetworkObject.Length > 0 && MinigameManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MinigameManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MinigameManagerBehavior>();
						}
					}
			...
			else if (obj is RopeNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (RopeNetworkObject.Length > 0 && RopeNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(RopeNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<RopeBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateMap instead, its shorter and easier to type out ;)")]
		public MapBehavior InstantiateMapNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MapNetworkObject[index]);
			var netBehavior = go.GetComponent<MapBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MapBehavior>().networkObject = (MapNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateMinigameManager instead, its shorter and easier to type out ;)")]
		public MinigameManagerBehavior InstantiateMinigameManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MinigameManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<MinigameManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MinigameManagerBehavior>().networkObject = (MinigameManagerNetworkObject)obj;
		...
		[Obsolete("Use InstantiateRope instead, its shorter and easier to type out ;)")]
		public RopeBehavior InstantiateRopeNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RopeNetworkObject[index]);
			var netBehavior = go.GetComponent<RopeBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<RopeBehavior>().networkObject = (RopeNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Map
		/// </summary>
		/// <returns>
		/// A local instance of MapBehavior
		/// </returns>
		/// <param name="index">The index of the Map prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MapBehavior InstantiateMap(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (MapNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Map. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of MinigameManager
		/// </summary>
		/// <returns>
		/// A local instance of MinigameManagerBehavior
		/// </returns>
		/// <param name="index">The index of the MinigameManager prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MinigameManagerBehavior InstantiateMinigameManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (MinigameManagerNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: MinigameManager. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of Rope
		/// </summary>
		/// <returns>
		/// A local instance of RopeBehavior
		/// </returns>
		/// <param name="index">The index of the Rope prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public RopeBehavior InstantiateRope(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (RopeNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Rope. Add them at the NetworkManager prefab.");
				return null;
			}
